...
--- The total code coverage of code is 80%, which includes fakenews\config.py, fakenews\data\make_dataset.py, fakenews\data\preprocessing.py, fakenews\model\model.py, fakenews\model\train_model.py and all init and test files. We are not at 100% coverage of our code as many advanced interaction of functions are wrapped in other functions and are hard to test just like that. Despite that, the base functionality of calculation and processings steps are widely covered by unit tests. ---
...
--- We did make use of DVC in the following way: Initially, we used dvc in combination with Google Drive were we stored the version controlled data in form of hashed files. Like that we only pushed data.dvc into github and tagged the current data version. We decided to dvc the whole data folder, as we wanted to track all changes and didn't want to have any dvc file inconsistencies. In the end it helped us in saving storage space for controlling changes of parts of our data set. ---
...
--- We have organized our continuous integration into 3 separate files: one for doing code formatting and linting with ruff, one for running a docker image on PR and one for testing. In particular for our running the unit tests on the data part, we used a dvc workflow which required storing google drive credentials in GitHub secrets, and is used to test the actual current version of the data related with the PR. An example of a triggered workflow can be seen for the docker image. On PR on main we trigger to build a docker image for training and push it to docker Hub, which is also enabled with storing the credentials in GitHub secrets. ---
...
```
as our script is completely automated with make commands.
...
We can change configs over command line inputs. While running the experiment, the configs used are stored in the `outputs/` folder, which stores named according to the date and time the experiment is run, the configs, hydra settings and overriders yamls.
...
![my_figure](figures/artifacts.png) ---
